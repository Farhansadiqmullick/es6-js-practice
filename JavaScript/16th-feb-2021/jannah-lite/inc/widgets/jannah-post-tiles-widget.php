...
add_action('widgets_init', 'jannah_post_tiles_widget');
function jannah_post_tiles_widget() {
    register_widget('jannah_post_tiles_widget');
}

class jannah_post_tiles_widget extends WP_Widget{

    public function __construct(){
        parent::__construct('jannah_post_tiles_widget',esc_html__('Jannah Post Tiles','jannah-lite'),array(
            'description' => __('Post tiles by 4 post','jannah-lite'),
        ));
    ...
    public function widget($arg,$instance){
        ...
        $before_posts  = '<div class="posts-tile-items widget-posts-wrapper">';
        $after_posts   = '</div>';
                    ...
                    <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                        ...
                        <div class="thumb-overlay">
                            ...
                            if(!empty($cats)){
                                ...
                                foreach($cats as $cat){
                                    if(count($cats_id) == 1 && $cat->slug == $cats_id[0]){
