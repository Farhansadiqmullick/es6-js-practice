...
add_action('widgets_init', 'jannah_post_block_5');
function jannah_post_block_5() {
    register_widget('jannah_post_block_5');
}

class jannah_post_block_5 extends WP_Widget{

    public function __construct(){
        parent::__construct('jannah_post_block_5',esc_html__('Jannah Post Block 5','jannah-lite'),array(
            'description' => __('Post block style 5','jannah-lite'),
        ));
    ...
    public function widget($arg,$instance){
        ...
        $before_posts  = '<div class="posts-block-5-items widget-posts-wrapper">';
        $after_posts   = '</div>';
                ...
                while( $query->have_posts() ){
                    ...
                    <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                        <?php if($featured_1_done == false){ ?>
                            <div class="post-thumb-wrapper">
                                ...
                                <?php if ( has_post_thumbnail() ){ ?>
                                    <?php the_post_thumbnail('jannah_big_thumb', array('class' => 'img-fluid')); ?>
                                <?php } ?>
                                <?php if ( has_post_format( 'video' )){ echo '<span class="video-play-icon"><i class="fas fa-play"></i></span>'; } ?>
                                </a>
                            </div>
                        ...
                        $featured_1_done = true;
                            ...
                            <?php } else { ?>
                                <span class="thumb-placehold"></span>
                            <?php } ?>
                            <?php if ( has_post_format( 'video' )){ echo '<span class="video-play-icon"><i class="fas fa-play"></i></span>'; } ?>
                            </a>
        ...
        if ( ! empty( $bg_color ) || ! empty( $font_color ) ){
            ...
            if ( ! empty( $font_color ) ){
                $out .= "
                    ...
                    $widget_id .posts-block-5-items .post .post-title a:hover,
                    $widget_id .posts-block-5-items .post:first-child a.post-meta:hover {
                        color: $font_color;
                    }
            ...
            if( !empty( $bg_color ) ) {
                $out .= "
                    ...
                    $widget_id .posts-block-5-items .post .post-date,
                    $widget_id .posts-block-5-items .post .post-title{
                        color: white;
                    }
                ";
            }
